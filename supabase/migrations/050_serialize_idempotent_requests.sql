-- Serialize concurrent idempotent requests via advisory lock.
-- Acquires a transaction-level lock keyed on the idempotency UUID so that
-- duplicate requests block instead of hitting a unique-constraint error.

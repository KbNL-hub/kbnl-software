-- Add product, ATC, order_no, child_order_no to get_active_mdd_trucks_remaining RPC
-- so the Monitor Trucks MDD section can display what each trip is carrying.
...
RETURNS json LANGUAGE sql STABLE AS $$
  ...
  FROM (
    SELECT
      ...
      r.order_no,
      r.child_order_no,
      GREATEST(r.loaded_quantity - COALESCE(s.total_offloaded, 0) - COALESCE(d.total_shortage, 0) - COALESCE(d.total_caked, 0), 0) AS remaining,

...
export default function TruckAdminDashboard() {
  ...
  const [tab, setTab] = useState<"reports" | "procurement" | "balance">("reports")
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: admin } = await supabase
        .from("truck_admins").select("admin_id, full_name").eq("admin_id", user.id).single()
    ...
    const enriched = await Promise.all(
      procRaw.map(async (p) => {
        const { data: dists } = await supabase
          .from("procurement_distributions").select("plate_number, amount_allocated").eq("procurement_id", p.procurement_id)
  ...
  async function handleDeposit() {
    ...
    const { error } = await supabase.from("maintenance_deposits").insert([{
      amount,
  ...
  async function handleLogProcurement() {
    ...
    if (procErr || !procurement) { setProcError("Failed to log procurement"); return }
    ...
    <div style={{ minHeight: "100vh", background: "#f9f9f9", fontFamily: "Arial" }}>
      ...
      <div style={{ padding: 24, maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          {(["reports", "procurement", "balance"] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              style={{
            ...
            >
              {t === "reports" ? "Maintenance Reports" : t === "procurement" ? "Bulk Procurement" : "Top Up Balance"}
            </button>
          ))}
        </div>
                  ...
                  <div key={r.report_id} style={{ background: "white", border: `1px solid ${r.status === "Rejected" ? "#ff444433" : "#eee"}`, borderRadius: 10, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                    ...
                    {r.status === "Pending" && (
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button onClick={() => setValidating(r)} style={{ flex: 1, padding: "8px 0", background: "#00aa00", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold", fontSize: 13 }}>
                          Validate
                        </button>
                        <button onClick={() => { setRejecting(r); setRejectReason(""); setRejectError("") }} style={{ flex: 1, padding: "8px 0", background: "white", color: "#ff4444", border: "1px solid #ff4444", borderRadius: 6, cursor: "pointer", fontWeight: "bold", fontSize: 13 }}>
                          Reject
                        </button>
                      </div>
                    )}
        ...
        {tab === "procurement" && (
          <div style={{ maxWidth: 600 }}>
            ...
            <button onClick={handleLogProcurement} disabled={procLoading} style={{ width: "100%", padding: "12px 0", background: "#0070f3", color: "white", border: "none", borderRadius: 6, cursor: procLoading ? "not-allowed" : "pointer", fontWeight: "bold", fontSize: 15 }}>
        ...
        {tab === "balance" && (
          <div style={{ maxWidth: 480 }}>
            <h3 style={{ marginBottom: 20 }}>Top Up Maintenance Balance</h3>
            <div style={{ background: "white", border: "1px solid #eee", borderRadius: 12, padding: 20, marginBottom: 24 }}>
              ...
              <p style={{ margin: 0, fontSize: 28, fontWeight: "bold", color: "#0070f3" }}>
                ₦{maintenanceBalance !== null ? maintenanceBalance.toLocaleString() : "—"}
              </p>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                type="text" inputMode="numeric" placeholder="e.g. 500,000"
                value={depositAmount}
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <input
                type="text" placeholder="e.g. Monthly allocation"
                value={depositNote}
            ...
            {depositError && <p style={errorStyle}>{depositError}</p>}
            <button onClick={handleDeposit} disabled={depositLoading} style={{ width: "100%", padding: "12px 0", background: "#0070f3", color: "white", border: "none", borderRadius: 6, cursor: depositLoading ? "not-allowed" : "pointer", fontWeight: "bold", fontSize: 15 }}>
        ...
        <div style={overlay}>
          <div style={modal}>
            ...
            <div style={{ background: "#f9f9f9", borderRadius: 8, padding: 16, marginBottom: 16 }}>
              ...
              {validating.maintenance_location && (
                <p style={{ margin: "0 0 8px" }}><strong>Location:</strong> {validating.maintenance_location}</p>
              )}
            ...
            <p style={{ fontSize: 13, color: "#888", marginBottom: 24 }}>
              ...
              {(maintenanceBalance ?? 0) - validating.amount < 0 && (
                <span style={{ color: "#ff4444", marginLeft: 8, fontSize: 12 }}>⚠️ Insufficient balance</span>
              )}
            </p>

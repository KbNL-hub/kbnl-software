    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: isMobile ? "16px" : "32px", fontFamily: "'Inter', sans-serif" }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "64px 24px", background: "white", borderRadius: 12, border: "1px solid #e2e8f0", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.05)" }}>
          ...
          <h3 style={{ margin: "0 0 8px", color: "#0f172a", fontSize: FONT_SIZE.xl, fontWeight: 600 }}>No truck admins yet</h3>
          <p style={{ color: "#64748b", fontSize: FONT_SIZE.base, margin: "0 0 24px", maxWidth: 400, marginLeft: "auto", marginRight: "auto" }}>Add truck admins to manage fleet administration tasks.</p>
                  ...
                  <div key={admin.admin_id} style={{ background: "white", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0, 0, 0, 0.05)", transition: "all 0.2s ease" }} onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 12px rgba(0, 0, 0, 0.08)"; e.currentTarget.style.borderColor = "#cbd5e1" }} onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 1px 3px rgba(0, 0, 0, 0.05)"; e.currentTarget.style.borderColor = "#e2e8f0" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0 }}>
                        <div style={{ width: 40, height: 40, borderRadius: "50%", background: admin.profile_picture_url ? "transparent" : "linear-gradient(135deg, #0070f3 0%, #0056d4 100%)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600, fontSize: FONT_SIZE.md, flexShrink: 0, overflow: "hidden" }}>
                          {admin.profile_picture_url ? (
                                <Image src={admin.profile_picture_url} alt={admin.full_name} width={36} height={36} unoptimized style={{ objectFit: "cover" }} />
                          ) : (
                            admin.full_name.charAt(0).toUpperCase()
                          )}
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <h3 style={{ margin: "0 0 4px 0", color: "#0f172a", fontSize: FONT_SIZE.lg, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{admin.full_name}</h3>
                          <p style={{ margin: 0, color: "#64748b", fontSize: FONT_SIZE.sm, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{admin.phone_number || "No phone number"}</p>
                        </div>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                      <button onClick={() => { if (!canEdit) return; setEditingAdmin(admin); setEditName(admin.full_name); setEditPhone(admin.phone_number || ""); setMessage("") }} disabled={!canEdit} style={{ flex: 1, padding: "8px 12px", cursor: canEdit ? "pointer" : "not-allowed", borderRadius: 6, border: "1px solid #e2e8f0", color: canEdit ? "#0070f3" : "#94a3b8", background: canEdit ? "#f0f7ff" : "#e2e8f0", fontSize: FONT_SIZE.sm, fontWeight: 500, transition: "all 0.2s" }} onMouseEnter={e => { if (!canEdit) return; e.currentTarget.style.background = "#e0efff"; e.currentTarget.style.borderColor = "#0070f3" }} onMouseLeave={e => { if (!canEdit) return; e.currentTarget.style.background = "#f0f7ff"; e.currentTarget.style.borderColor = "#e2e8f0" }}>
                        Edit
                      </button>
                      <button onClick={() => { if (!canEdit) return; setDeletingId(admin.admin_id); setMessage("") }} disabled={!canEdit} style={{ flex: 1, padding: "8px 12px", cursor: canEdit ? "pointer" : "not-allowed", borderRadius: 6, border: "1px solid #fee2e2", color: canEdit ? "#ef4444" : "#94a3b8", background: canEdit ? "#fef2f2" : "#e2e8f0", fontSize: FONT_SIZE.sm, fontWeight: 500, transition: "all 0.2s" }} onMouseEnter={e => { if (!canEdit) return; e.currentTarget.style.background = "#fee2e2" }} onMouseLeave={e => { if (!canEdit) return; e.currentTarget.style.background = "#fef2f2" }}>
                        Delete
                      </button>
                    </div>
                  </div>
                      ...
                      <tr key={admin.admin_id} style={{ borderBottom: idx === admins.length - 1 ? "none" : "1px solid #e2e8f0", transition: "background 0.2s ease" }} onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <td style={{ padding: "12px 16px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: "50%", background: admin.profile_picture_url ? "transparent" : "linear-gradient(135deg, #0070f3 0%, #0056d4 100%)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600, fontSize: FONT_SIZE.base, flexShrink: 0, overflow: "hidden" }}>
                              {admin.profile_picture_url ? (
                            <Image src={admin.profile_picture_url} alt={admin.full_name} width={40} height={40} unoptimized style={{ objectFit: "cover" }} />
                              ) : (
                                admin.full_name.charAt(0).toUpperCase()
                              )}
                            </div>
                            <span style={{ color: "#0f172a", fontSize: FONT_SIZE.base, fontWeight: 500 }}>{admin.full_name}</span>
                          </div>
                        </td>
                        ...
                        <td style={{ padding: "12px 16px", textAlign: "right" }}>
                          <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                            <button onClick={() => { if (!canEdit) return; setEditingAdmin(admin); setEditName(admin.full_name); setEditPhone(admin.phone_number || ""); setMessage("") }} disabled={!canEdit} style={{ padding: "6px 10px", cursor: canEdit ? "pointer" : "not-allowed", borderRadius: 5, border: "1px solid #e2e8f0", color: canEdit ? "#0070f3" : "#94a3b8", background: canEdit ? "#f0f7ff" : "#e2e8f0", fontSize: FONT_SIZE.sm, fontWeight: 500, transition: "all 0.2s", minHeight: 32, minWidth: 32, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e => { if (!canEdit) return; e.currentTarget.style.background = "#e0efff"; e.currentTarget.style.borderColor = "#0070f3" }} onMouseLeave={e => { if (!canEdit) return; e.currentTarget.style.background = "#f0f7ff"; e.currentTarget.style.borderColor = "#e2e8f0" }}>
                              Edit
                            </button>
                            <button onClick={() => { if (!canEdit) return; setDeletingId(admin.admin_id); setMessage("") }} disabled={!canEdit} style={{ padding: "6px 10px", cursor: canEdit ? "pointer" : "not-allowed", borderRadius: 5, border: "1px solid #fee2e2", color: canEdit ? "#ef4444" : "#94a3b8", background: canEdit ? "#fef2f2" : "#e2e8f0", fontSize: FONT_SIZE.sm, fontWeight: 500, transition: "all 0.2s", minHeight: 32, minWidth: 32, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e => { if (!canEdit) return; e.currentTarget.style.background = "#fee2e2" }} onMouseLeave={e => { if (!canEdit) return; e.currentTarget.style.background = "#fef2f2" }}>
                              Delete
                            </button>
                          </div>
                        </td>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 100, padding: isMobile ? 0 : 24, animation: "fadeIn 0.2s ease-out" }}>
          ...
          <div style={{ background: "white", borderRadius: isMobile ? "20px 20px 0 0" : 12, padding: isMobile ? "28px 20px" : 32, width: "100%", maxWidth: 420, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)", animation: "slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1)" }}>
                ...
                {inviteResult ? (
                  ...
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                      <h3 style={{ margin: 0, color: "#0f172a", fontSize: FONT_SIZE.xl, fontWeight: 700 }}>Add Truck Admin</h3>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                      <div>
                        ...
                        <ModernInput type="text" placeholder="e.g. John Doe" value={fullName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setFullName(e.target.value); setMessage("") }} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") phoneRef.current?.focus() }} readOnly={!canEdit} style={{ width: "100%", padding: "12px 14px", boxSizing: "border-box", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: FONT_SIZE.base, background: "white", color: "#171717", minHeight: 48, transition: "border-color 0.2s ease" }} autoFocus />
                      </div>
                      <div>
                        <label style={{ display: "block", marginBottom: 6, color: "#475569", fontSize: FONT_SIZE.sm, fontWeight: 500 }}>Phone Number</label>
                        <ModernInput ref={phoneRef} type="text" placeholder="e.g. 08012345678" value={phoneNumber} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setPhoneNumber(e.target.value); setMessage("") }} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") emailRef.current?.focus() }} readOnly={!canEdit} style={{ width: "100%", padding: "12px 14px", boxSizing: "border-box", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: FONT_SIZE.base, background: "white", color: "#171717", minHeight: 48, transition: "border-color 0.2s ease" }} />
                      </div>
                      <div>
                        <label style={{ display: "block", marginBottom: 6, color: "#475569", fontSize: FONT_SIZE.sm, fontWeight: 500 }}>Email Address *</label>
                        <ModernInput ref={emailRef} type="email" placeholder="e.g. admin@example.com" value={email} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setEmail(e.target.value); setMessage("") }} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") handleInvite() }} readOnly={!canEdit} style={{ width: "100%", padding: "12px 14px", boxSizing: "border-box", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: FONT_SIZE.base, background: "white", color: "#171717", minHeight: 48, transition: "border-color 0.2s ease" }} />
                      </div>
                    </div>
            ...
            {editingAdmin && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                  <h3 style={{ margin: 0, color: "#0f172a", fontSize: FONT_SIZE.xl, fontWeight: 700 }}>Edit Admin</h3>
                  <button onClick={closeModals} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", padding: 0, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#64748b"} onMouseLeave={e => e.currentTarget.style.color = "#94a3b8"}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                  <div>
                    <label style={{ display: "block", marginBottom: 6, color: "#475569", fontSize: FONT_SIZE.sm, fontWeight: 500 }}>Full Name *</label>
                    <ModernInput type="text" value={editName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setEditName(e.target.value); setMessage("") }} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") editPhoneRef.current?.focus() }} readOnly={!canEdit} style={{ width: "100%", padding: "12px 14px", boxSizing: "border-box", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: FONT_SIZE.base, background: "white", color: "#171717", minHeight: 48, transition: "border-color 0.2s ease" }} autoFocus />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 6, color: "#475569", fontSize: FONT_SIZE.sm, fontWeight: 500 }}>Phone Number</label>
                    <ModernInput ref={editPhoneRef} type="text" value={editPhone} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setEditPhone(e.target.value); setMessage("") }} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") handleUpdate() }} readOnly={!canEdit} style={{ width: "100%", padding: "12px 14px", boxSizing: "border-box", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: FONT_SIZE.base, background: "white", color: "#171717", minHeight: 48, transition: "border-color 0.2s ease" }} />
                  </div>
                </div>
                {message && <div style={{ padding: 12, background: "#fef2f2", borderLeft: "4px solid #ef4444", borderRadius: 4, marginBottom: 20, color: "#b91c1c", fontSize: FONT_SIZE.sm }}>{message}</div>}
            ...
            {deletingId && (
              <>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                  ...
                  <h3 style={{ margin: "0 0 12px", color: "#0f172a", fontSize: FONT_SIZE.xl, fontWeight: 700 }}>Delete Truck Admin</h3>
                  <p style={{ margin: "0 0 20px", color: "#64748b", fontSize: FONT_SIZE.base, lineHeight: 1.5 }}>Are you sure? This action cannot be undone.</p>

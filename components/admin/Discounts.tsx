...
export default function Discounts() {
  ...
  const backfillHistoricalDiffs = useCallback(async () => {
    try {
      // Step 1: Cleanup duplicates — keep only the most recent record per source
      ...
      if (allRecords && allRecords.length > 0) {
        ...
        for (const id of dupes) {
          await apiMutate("finance", { action: "delete", table: "price_adjustments", filters: { id } })
        }
      }
      ...
      const existingMap = new Map(
        (existing || []).map(e => [`${e.source_type}:${e.source_id}`, e.status])
      )

      // Step 3: Backfill stops
      const { data: confirmations } = await supabase
        .from("Stop_Confirmations")
        .select("stop_id, price_per_bag, company_price, area, broker_id, price_reason")
        .not("company_price", "is", null)

      if (confirmations) {
        const stopIds = confirmations.map(c => c.stop_id)
        const { data: stops } = await supabase
          .from("Stops")
          .select("stop_id, trip_id, discount_status")
          .in("stop_id", stopIds)
        ...
        if (tripIds.length > 0) {
          const { data: trips } = await supabase
            .from("Trips")
            .select("trip_id, product")
            .in("trip_id", tripIds)
          for (const t of trips || []) tripMap.set(t.trip_id, t.product)
        }

        for (const c of confirmations) {
          if (c.company_price && c.price_per_bag && c.price_per_bag !== c.company_price) {
            const key = `stop:${c.stop_id}`
            const existingStatus = existingMap.get(key)
            if (existingStatus) continue

            const stop = stopMap.get(c.stop_id)
            const product = stop ? tripMap.get(stop.trip_id) || "Unknown" : "Unknown"
            const reason = c.price_reason || "Historical price difference"
            const isHistorical = stop?.discount_status === "none" || !stop?.discount_status
            ...
            await apiMutate("finance", {
              ...
              data: {
                ...
                area: c.area,
                product,
                company_price: c.company_price,
                adjusted_price: c.price_per_bag,
                price_reason: reason,
                status: isHistorical ? "Approved" : "Pending",
              },
            })
            existingMap.set(key, isHistorical ? "Approved" : "Pending")
          }
        }
      }

      // Step 4: Backfill store_sales with company_price
      const { data: sales } = await supabase
        .from("store_sales")
        .select("sale_id, price_per_bag, company_price, area, broker_id, product, price_reason, status, group_id")
        .not("company_price", "is", null)

      if (sales) {
        for (const s of sales) {
          if (s.company_price && s.price_per_bag && s.price_per_bag !== s.company_price) {
            const key = `store_sale:${s.sale_id}`
            const existingStatus = existingMap.get(key)
            if (existingStatus) continue

            const reason = s.price_reason || "Historical price difference"
            const isHistorical = s.status === "Confirmed"
            ...
            await apiMutate("finance", {
              ...
              data: {
                ...
                area: s.area,
                product: s.product,
                company_price: s.company_price,
                adjusted_price: s.price_per_bag,
                price_reason: reason,
                status: isHistorical ? "Approved" : "Pending",
      ...
      // Step 5: Backfill store_sales with NULL company_price
      const { data: salesNoCp } = await supabase
        .from("store_sales")
        .select("sale_id, price_per_bag, area, broker_id, product, price_reason, status, group_id")
        .is("company_price", null)
        .not("price_per_bag", "is", null)

      if (salesNoCp && salesNoCp.length > 0) {
        const { data: cpData } = await supabase
          .from("company_prices")
          .select("area, product, price")
          .in("area", [...new Set(salesNoCp.map(s => s.area))])
        const cpMap = new Map<string, number>()
        for (const cp of cpData || []) cpMap.set(`${cp.area}||${cp.product}`, cp.price)

        for (const s of salesNoCp) {
          const cp = cpMap.get(`${s.area}||${s.product}`)
          if (cp && s.price_per_bag !== cp) {
            const key = `store_sale:${s.sale_id}`
            const existingStatus = existingMap.get(key)
            if (existingStatus) continue
            ...
            await apiMutate("finance", {
              action: "update",
              table: "store_sales",
              data: { company_price: cp },
              filters: { sale_id: s.sale_id },
            })
  ...
  useEffect(() => {
    fetchAdjustments().then(() => backfillHistoricalDiffs()).then(() => fetchAdjustments())
  }, [fetchAdjustments, backfillHistoricalDiffs])
